...
func (h *oauth2Handler) serveJWKS(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	// [x] Take the JWKS from the condifured cache (firestore DB as of now) for the Ivmanto OID provider record
	pks, err := h.server.Pks.GetPKSCache("ivmanto")
	if err != nil {
		_ = level.Error(h.logger).Log("method", "GetPKSCache", "error", err)
		h.server.responseIntServerError(w, "serverJWKS", err)
		return
	}

	if pks == nil {
		_ = level.Error(h.logger).Log("unexpected-null-pks", fmt.Sprintf("pks == nil: %#v", pks == nil))
		h.server.responseIntServerError(w, "serverJWKS", fmt.Errorf("pks in cache is NULL"))
		return
	}

	var jk = *pks.Jwks
	_ = level.Info(h.logger).Log("pks-JWKS", fmt.Sprintf("jwks %+v", jk))

	resp, err := json.MarshalIndent(jk, "", " ")
	if err != nil {
		_ = level.Error(h.logger).Log("method", "MarshalIndent", "error", err)
		h.server.responseIntServerError(w, "serverJWKS", err)
		return
	}
	...
	_, err = w.Write(resp)
	if err != nil {
		_ = level.Error(h.logger).Log("method", "response-w.Write", "error", err)
	}
}
